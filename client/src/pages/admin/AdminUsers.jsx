    ...
    <div className="w-full mt-6 space-y-6">
      <div className="px-4">
        ...
        <p className="text-sm text-gray-600 mb-6">
          Total Users: {users.length} | Admins: {users.filter((u) => u.role === 'admin').length} | Active:{' '}
              ...
              <tbody>
                {users.map((user) => (
                  <tr key={user._id} className="border-b dark:border-gray-700">
                    <td className="p-3 break-words">{user.name || 'N/A'}</td>
                    <td className="p-3 break-words">{user.email || 'N/A'}</td>
                    <td className="p-3">
                      <span
